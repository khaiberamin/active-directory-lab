...
- In Host-Only Properties, double click Internet Protocol Version 4 (TCP/IPv4) to enter IPv4 Properties tab. Then, set up custom IP address rather than an automatic one. IP address should be 192.168.56.x with a 255.255.255.0 subnet mask. Leave default gateway blank, and set Preferred DNS server to the same as the IP address. ![Text](../screenshots/active-directory-setup/assign-IP-address.png)
- use ipconfig in command prompt to verify IP addresses have been updated ![Text](../screenshots/active-directory-setup/ipconfig-IP-address-confirmation.png)
...
- After reboot, open Tools -> Active Directory Users and Computers. Then expand homelab.local and open the Domain Controllers folder to confirm that DC01 Computer is listed as the Domain Controller. ![Text](../screenshots/active-directory-setup/promoted-DC-confirmation.png)
